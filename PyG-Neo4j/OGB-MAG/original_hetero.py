...
if not args.use_hgt_loader:
    ...
    val_loader = HGTLoader(data, num_samples=[1024] * 4,
                           input_nodes=val_input_nodes, **kwargs)
    test_loader = HGTLoader(data, num_neighbors=[1024] * 2,
                                input_nodes=test_input_nodes, **kwargs)
...
st_time_training = time.time()
for epoch in range(1, 21):
    loss = train()
    val_acc = test(val_loader)
    print(f'Epoch: {epoch:02d}, Loss: {loss:.4f}, Val: {val_acc:.4f}')

print("Time to train%s seconds" % (time.time() - st_time_training))

st_time_testing = time.time()
test_acc = test(test_loader)
print(f'Accuracy: {test_acc:.4f}')
print("Time to test %s seconds" % (time.time() - st_time_testing))